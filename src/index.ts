...
/** One Smithery market server row. */
interface MarketServerRow {
  id: string
  name: string
  description: string
  verified: boolean
...
const marketCache = new Map<string, { at: number; rows: MarketServerRow[] | null }>()
 ...
 * readable error.
 */
async function fetchSmitheryMarket(query: string): Promise<MarketServerRow[] | null> {
  const q = query.trim()
  const key = q === '' ? '*' : q
  const cached = marketCache.get(key)
  if (cached !== undefined && Date.now() - cached.at < MARKET_CACHE_MS) return cached.rows
  const listUrl = q === ''
    ? 'https://registry.smithery.ai/servers'
    : `https://registry.smithery.ai/servers?q=${encodeURIComponent(q)}`
  let rows: MarketServerRow[] | null = null
  try {
    const res = await fetch(listUrl, { signal: AbortSignal.timeout(20000) })
    if (!res.ok) return null
    const json = await res.json() as { servers?: Array<{ qualifiedName: string; displayName: string; description: string; verified?: boolean; useCount?: number }> }
    const listed = (json.servers ?? []).slice(0, 10)
    // detail fetches with bounded concurrency (4) and a per-request timeout
    const out: MarketServerRow[] = new Array(listed.length)
    let next = 0
    const worker = async (): Promise<void> => {
      while (true) {
        ...
        try {
          ...
          const dj = await d.json() as { connections?: Array<{ type?: string; deploymentUrl?: string }> }
