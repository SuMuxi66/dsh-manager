...
function KeysTab({ busy, setBusy, msg, setMsg }) {
    ...
    const refresh = async () => {
        await loadInto((data) => { setAvailable(data.available); setKeys(data.keys); }, (msg) => setMsg({ ok: false, text: msg }), async () => {
            const data = await managerFetch('/manager/api/keys');
            if (!data.ok)
                throw new Error(data.ok === undefined ? 'host 端未加载 dsh-manager（请重启 web/桌面端）' : 'keys API 错误');
            return data;
        });
    };
    useEffect(() => { void refresh().then(() => setLoading(false)); }, []);
