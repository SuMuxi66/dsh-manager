...
function PluginsTab({ busy, setBusy, msg, setMsg }) {
    ...
    const refresh = async () => {
        await loadInto((data) => { setProfile(data.profile); setPlugins(data.plugins); }, (msg) => setMsg({ ok: false, text: msg }), async () => {
            const data = await managerFetch('/manager/api/plugins');
            if (!data.ok)
                throw new Error(data.ok === undefined ? 'host 端未加载 dsh-manager（请重启 web/桌面端）' : 'plugins API 错误');
            return data;
        });
    };
...
function MarketTab({ busy, setBusy, msg, setMsg }) {
    ...
    const load = async () => {
        await loadInto((data) => setMarket(data.plugins), (msg) => setMsg({ ok: false, text: msg }), async () => {
            const data = await managerFetch('/manager/api/market');
            if (!data.ok)
                throw new Error(data.ok === undefined ? 'host 端未加载 dsh-manager（请重启 web/桌面端）' : 'market API 错误');
            return data;
        });
    };
    useEffect(() => { void load(); }, []);
...
function SkillsTab({ busy, setBusy, msg, setMsg }) {
    ...
    const refresh = async () => {
        await loadInto((data) => setSkills(data.skills), (msg) => setMsg({ ok: false, text: msg }), async () => {
            const data = await managerFetch('/manager/api/skills');
            if (!data.ok)
                throw new Error(data.ok === undefined ? 'host 端未加载 dsh-manager（请重启 web/桌面端）' : 'skills API 错误');
            return data;
        });
    };
    useEffect(() => { void refresh().then(() => setLoading(false)); }, []);
...
function McpTab({ busy, setBusy, msg, setMsg }) {
    ...
    const refresh = async () => {
        await loadInto((data) => setServers(data.servers), (msg) => setMsg({ ok: false, text: msg }), async () => {
            const data = await managerFetch('/manager/api/mcp');
            if (!data.ok)
                throw new Error(data.ok === undefined ? 'host 端未加载 dsh-manager（请重启 web/桌面端）' : 'mcp API 错误');
            return data;
        });
    };
    useEffect(() => { void refresh().then(() => setLoading(false)); }, []);
...
function ThemeTab({ busy, setBusy, msg, setMsg }) {
    ...
    const [loading, setLoading] = useState(true);
    const refresh = async () => {
        await loadInto((data) => { setPreference(data.preference); setSkins(data.skins); }, (msg) => setMsg({ ok: false, text: msg }), async () => {
            const data = await managerFetch('/manager/api/theme');
            if (!data.ok)
                throw new Error(data.ok === undefined ? 'host 端未加载 dsh-manager（请重启 web/桌面端）' : 'theme API 错误');
            return data;
        });
    };
    useEffect(() => { void refresh().then(() => setLoading(false)); }, []);
...
function ManagerPanel() {
    ...
    return (_jsxs(_Fragment, { children: [_jsx("div", { className: "dshm-head", children: _jsx("h2", { children: "DSH \u7BA1\u7406\u63A7\u5236\u53F0" }) }), _jsx("div", { className: "dshm-tabs", children: tabs.map((t) => (_jsx("button", { type: "button", className: `dshm-tab${tab === t.id ? ' active' : ''}`, onClick: () => { setTab(t.id); setMsg(null); }, children: t.label }, t.id))) }), _jsxs("div", { className: "dshm-body", children: [tab === 'plugins' && _jsx(PluginsTab, { busy: busy, setBusy: setBusy, msg: msg, setMsg: setMsg }), tab === 'market' && _jsx(MarketTab, { busy: busy, setBusy: setBusy, msg: msg, setMsg: setMsg }), tab === 'skills' && _jsx(SkillsTab, { busy: busy, setBusy: setBusy, msg: msg, setMsg: setMsg }), tab === 'mcp' && _jsx(McpTab, { busy: busy, setBusy: setBusy, msg: msg, setMsg: setMsg }), tab === 'theme' && _jsx(ThemeTab, { busy: busy, setBusy: setBusy, msg: msg, setMsg: setMsg }), _jsx(Msg, { msg: msg })] })] }));
}
