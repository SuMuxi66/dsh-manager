...
function ModelsTab({ busy, setBusy, msg, setMsg }) {
    ...
    const refresh = async () => {
        ...
        }, (msg) => setMsg({ ok: false, text: msg }), async () => {
            const data = await managerFetch('/manager/api/models');
            if (!data.ok)
                throw new Error(data.ok === undefined ? 'host 端未加载 dsh-manager（请重启 web/桌面端）' : 'models API 错误');
            return data;
        });
    };
    useEffect(() => { void refresh().then(() => setLoading(false)); }, []);
...
function ManagerPanel({ onClose }) {
    ...
    return (_jsxs(_Fragment, { children: [_jsxs("div", { className: "dshm-head", children: [_jsx("h2", { children: "DSH \u7BA1\u7406\u63A7\u5236\u53F0" }), onClose !== undefined && (_jsx("button", { type: "button", className: "dshm-close", onClick: onClose, children: "\u5173\u95ED" }))] }), _jsx("div", { className: "dshm-tabs", children: tabs.map((t) => (_jsx("button", { type: "button", className: `dshm-tab${tab === t.id ? ' active' : ''}`, onClick: () => { setTab(t.id); setMsg(null); }, children: t.label }, t.id))) }), _jsxs("div", { className: "dshm-body", children: [tab === 'plugins' && _jsx(PluginsTab, { busy: busy, setBusy: setBusy, msg: msg, setMsg: setMsg }), tab === 'market' && _jsx(MarketTab, { busy: busy, setBusy: setBusy, msg: msg, setMsg: setMsg }), tab === 'skills' && _jsx(SkillsTab, { busy: busy, setBusy: setBusy, msg: msg, setMsg: setMsg }), tab === 'mcp' && _jsx(McpTab, { busy: busy, setBusy: setBusy, msg: msg, setMsg: setMsg }), tab === 'models' && _jsx(ModelsTab, { busy: busy, setBusy: setBusy, msg: msg, setMsg: setMsg }), tab === 'theme' && _jsx(ThemeTab, { busy: busy, setBusy: setBusy, msg: msg, setMsg: setMsg }), _jsx(Msg, { msg: msg })] })] }));
}
