...
async function scanSkills() {
	...
	try {
		const presetsDir = join(dirname(require.resolve("@deepseek-ai/dsh/package.json")), "config", "agent-presets");
		const presetDirs = await readdir(presetsDir, { withFileTypes: true });
		for (const entry of presetDirs) if (entry.isDirectory()) {
			...
			try {
				...
				if (req.method === "POST" && path === "/manager/api/skills/import-all") {
					...
					for (const skill of candidates) {
						if (skill.source === "user-dsh") continue;
						...
						try {
							await cp(skill.path.endsWith(`${sep}SKILL.md`) ? dirname(skill.path) : skill.path, join(targetDir, skill.name), { recursive: true });
