...
async function collectCredentialRefs(ctx) {
	const refs = /* @__PURE__ */ new Set();
	const settings = ctx.get("settings");
	const providers = ctx.get("llm")?.listConfigurableProviders?.() ?? [];
	for (const entry of providers) {
		const section = settings?.get?.(entry.settingsNs);
		if (section !== void 0 && typeof section.apiKeyEnv === "string" && section.apiKeyEnv !== "") refs.add(section.apiKeyEnv);
	}
	return [...refs];
}
/**
		...
		handler: async (req, res) => {
			...
			try {
				...
				if (req.method === "GET" && path === "/manager/api/keys") {
					...
					json(res, 200, {
						ok: true,
						available: credentials?.describe !== void 0,
						keys: rows
					});
					return;
				}
				if (req.method === "POST" && path === "/manager/api/keys/set") {
					...
					if (value === "") {
						json(res, 400, {
							ok: false,
							error: "value required (use unset to clear)"
						});
						return;
					}
					const credentials = ctx.get("credentials");
					if (credentials?.set === void 0) {
						json(res, 500, {
							ok: false,
				...
				if (req.method === "GET" && path === "/manager/api/models") {
					...
					json(res, 200, {
						ok: true,
						default: settings?.get?.("agent-default-model") ?? null,
						providers: (llm?.listConfigurableProviders?.() ?? []).map((entry) => {
							const sectionView = { ...settings?.get?.(entry.settingsNs) ?? {} };
							return {
				...
				if (req.method === "POST" && path === "/manager/api/models/provider") {
					...
					if (ns === "" || section === null) {
						json(res, 400, {
							ok: false,
							error: "settingsNs and section required"
						});
						return;
					}
					if (!(ctx.get("llm")?.listConfigurableProviders?.() ?? []).some((entry) => entry.settingsNs === ns)) {
						json(res, 403, {
							ok: false,
							error: `settings namespace ${JSON.stringify(ns)} is not a configurable provider`
						});
						return;
					}
					const settings = ctx.get("settings");
