...
| **Skills 管理** | 文件系统技能目录扫描（`$DSH_HOME/skills`、`~/.agents/skills`、bundled），列表/详情/安装（写入用户目录）/卸载（仅用户目录），防目录穿越 |
| **MCP 管理** | 已加载 MCP 服务器实时状态（stdio / streamable-http）、新增/编辑/删除（写入 profile `cordis.patch.yml` 托管块）、env/header 密钥不回显（留空保留、× 删除、输入覆盖） |
| **Key 管理** | 识别模型供应商的 `apiKeyEnv` 引用，`describe` 只回显已配置/来源/可写状态（**值永不回显**），设置/清除走 credentials 服务 |
...
- **Host 半**（`src/index.ts`）：注册 `/manager/api` 路由（plugins / market / toggle /
  install / uninstall / skills / mcp / keys / models / theme），通过 `ctx.loader`、
