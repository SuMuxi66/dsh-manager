...
页面**（侧边栏「设置」→ 左侧页面列表），提供插件管理、插件市场、Skills 管理、
MCP 管理（含开源商店）与主题/皮肤管理。Web 端与桌面端通用（共享 `~/.dsh/profiles/web`）。
...
重启 web / 桌面端后：侧边栏「设置」→ 左侧页面列表出现「管理控制台」。
...
| **Skills 管理** | 文件系统技能目录扫描（`$DSH_HOME/skills`、`~/.agents/skills`、bundled），列表/详情/新建/卸载（仅用户目录）；**用户技能存于 `~/.dsh/skills`**，支持导入 .md 技能文件（名称取 frontmatter，否则取文件名，自动规范化），防目录穿越 |
...
- API 套件：`node scripts/m2m3-api-test.mjs`（28 断言：skills 导入/落盘路径/详情、MCP 配置、Smithery 市场搜索与一键安装、theme；keys/models 端点已移除）
- UI 套件：`BASE=<port> node scripts/ui-e2e.mjs`（20 断言：全部 5 个页签 + md 文件导入 + MCP 商店搜索）

## 架构

- **Host 半**（`src/index.ts`）：注册 `/manager/api` 路由（plugins / market / toggle /
  install / uninstall / skills / skills-import / mcp / mcp-market / theme），
...
- **Client 半**（`src/client/index.tsx`）：注册官方 **`settings.section`** 槽位
  （设置面板内嵌「管理控制台」页面），渲染 ManagerPanel，数据走同源
